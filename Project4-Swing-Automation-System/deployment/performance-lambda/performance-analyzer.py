...
def send_performance_email_notification(report, results, days_back):
    """Send comprehensive performance analysis email"""
    try:
        subject = f"📈 Performance Analysis Report - {datetime.now().strftime('%Y-%m-%d')}"
        
        # Calculate key metrics
        total_signals = len(results)
        successful_signals = len([r for r in results if r['success']]) if results else 0
        success_rate = (successful_signals / total_signals * 100) if total_signals > 0 else 0
        avg_profit = sum([r['profit_percentage'] for r in results]) / total_signals if results else 0
...
"""
        ...
        print(f"Performance analysis email sent successfully")
        
    except Exception as e:
        print(f"Error sending performance email: {e}")
...
def lambda_handler(event, context):
    ...
    if not signals:
        # Send email even when no signals found
        no_signals_report = f"""
🎯 SWING TRADING BOT PERFORMANCE REPORT
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

📊 ANALYSIS RESULT:
No trading signals found in the last {days_back} days.

This could mean:
• Market conditions have been neutral (no strong BUY/SELL signals)
• The system is working correctly but no opportunities detected
• All analyzed symbols are in HOLD status

The system continues to monitor daily for new opportunities.
"""
        send_performance_email_notification(no_signals_report, [], days_back)
    ...
    send_performance_email_notification(report, results, days_back)
